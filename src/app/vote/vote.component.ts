...
import { DialogComponent } from './dialong.component';
...
export class VoteComponent {
  ...
  async updateElo(winner: getImg, loser: getImg, expectedwinProbability: number, expectedLoserProbability: number) {
    ...
    this.openDialog(winner.score, winner.image, winnerNewRating, winner.name, winnerpop, pointpop, loser.score, kFactorWinner, kFactorLoser);
    if (checkwinner === true) {
  ...
  openDialog(winnerScore: number, winnerImageSrc: string, winnerrat: number
    ...
    const dialogRef = this.dialog.open(DialogComponent, {
      ...
      data: {
        winnerScore: winnerScore, winnerImageSrc: winnerImageSrc
        , Winnerrat: winnerrat, winnername: winnername, winnerpop: winnerpop
        , pointpop: pointpop, loserScore: loserScore, kFactorWinner: kFactorWinner, kFactorLoser: kFactorLoser
      }// ส่ง array ข้อมูลไปให้ Dialog
    });
